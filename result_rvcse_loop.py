...
from data_test import dataread_sciplex_test ,dataread_rvcse_other,dataread_rvcse_in,dataread_rvcse_other_loop,dataread_inter_other_loop
class dataload:
    def __init__(self,
                 data_inputx,
                 data_inputy,
...
def predict(y_true, y_pred):
        ...
        print('pearson', corr, "totalpearson", total_correlation, 'r2',r2,'mse', mse,'r2total',total_r2)
        # exit()

        return corr[0,1], total_correlation[0,1],r2,total_r2,mse

target =["Basal","Brush+PNEC","Cycling basal","Doublet","Ciliated","Hillock","Ionocyte",
                                        "Myoepithelial","Pre-ciliated","Secretory"]

# target =["B","CD4 T","CD8 T","Dendritic","Monocyte","NK","Plasma"]
# model_path = "savemodel/rvcse/modelnew_type29000.pt"
# model_path = "savemodel/modelnew_pla9000.pt"
model_path ="savemodel/bas_bru_train/model_Bas_Bas+PN_layer1.15.pt"
data_result = np.zeros((len(target),5))
...
for i in range (len(target)):
    ...
    x, y = dataread_rvcse_other_loop(file,target[i])
    # x, y = dataread_inter_other_loop(file, target[i])
    # dataread_inter_other_loop
    ...
    model = VAE()
    # model_path = "savemodel/rvcse/modelnew_type49000.pt"
    ...
    corr, total_correlation,r2,r2total,mse = predict(X_pre, out)
    print(i)
    data_result[i, 0] =corr
    data_result[i, 1] = total_correlation
    data_result[i, 2] =r2
    data_result[i, 3] = r2total
    data_result[i, 4] = mse
    # data_result[i,0],data_result[i,1],data_result[i,2],data_result[i,3],data_result[i,4]= predict(X_pre, out)
    # data_result[i,0] =


    np.savetxt("savemodel/type_interpla.csv", data_result, delimiter=",")
