...
class VAE(nn.Module):
    ...
    def __init__(self,
        ...
        self.ce_params = {
            ...
            "N_beta": n_beta,
            "K": n_causal,
            "L": latent_dim - n_causal,
            "z_dim": latent_dim,
            "M": n_labels,
        }

        self.causal_weight = 1e-8
        # self.causal_weight = 0
    ...
    def predic_causal(self, z: Tensor) -> Tensor:
        z_causal   = torch.split(z, self.canshu , dim=1)[0]

        pre_causal_result = self.predic_input(z_causal)
        pre_causal_result = self.predictor( pre_causal_result)
        pre_causal_result = self.final_pre( pre_causal_result)

        return pre_causal_result
    ...
    def hidden_pre(self,input:Tensor)-> Tensor:
        ...
        z_causal   = torch.split(z, self.canshu , dim=1)[0]
        z_non_causal   = torch.split(z, self.canshu , dim=1)[1]
        mu_causal = torch.split(mu, self.canshu , dim=1)[0]
        mu_non = torch.split(mu, self.canshu , dim=1)[1]
        log_causal  = torch.split(log_var, self.canshu , dim=1)[0]
        log_non = torch.split(log_var, self.canshu , dim=1)[1]
        return z,z_causal,z_non_causal,mu_causal,mu_non, log_causal, log_non, self.canshu
