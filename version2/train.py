...
from causal_effect import joint_uncond_v1 ,beta_info_flow_v1
class dataload:
    def __init__(self,
                 data_inputx,
                 data_inputy,
...
class Testdataload:
    ...
    def predict(self, new_model):
        ...
        type= torch.nan_to_num(type, nan=0.0)
...
def do_base_learning(model, wave, lr_inner, n_inner,t):
    new_model = VAE()
    ...
    for i in range(n_inner):
        ...
        # loss_causal = joint_uncond_v1 (new_model, y_true)
        ...
        if  t%50 == 0:
            causal_loss1 = joint_uncond_v1(model, y_true)
            causal_loss2 = beta_info_flow_v1(model, y_true)
            # print(causal_loss1,causal_loss2)

            causal_loss = causal_loss1 - 2.0 * causal_loss2
            # print(causal_loss)
            loss = loss + 0.5 * loss_y- 0.01* loss_x+ new_model.causal_weight*causal_loss
            # print(loss)
        # exit()
        else:
...
def reptile_sine(model, iterations, X_test, y_test, lr_inner=0.0001,
    ...
    for t in range(iterations):
        ...
        new_model = do_base_learning(model, wave, lr_inner, n_inner,t)
        ...
        if t % 500 == 0:
            ...
            for n_inner_1 in range(4):
                new_model1 = do_base_learning(model, wave1,
                                              lr_inner=0.001, n_inner=n_inner_1,t = t)
                # print(wave.)
            ...
            model_file_path = 'savemodel/model_Bas_Bas+PN_layer' + str(t / 10000) +'.pt'
